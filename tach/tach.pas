...
{:165}{173:}
  FONTINSHORTD: INTEGER;{:173}{181:}
  DEPTHTHRESHO: INTEGER;
...
{:825}{828:}
  ...
  MINIMALDEMER: ARRAY[0..3] OF INTEGER;
  MINIMUMDEMER: INTEGER;
...
END;
{:263}{518:}
PROCEDURE PRINTFILENAM(N,A,E:INTEGER);
BEGIN
  SLOWPRINT(A);
  SLOWPRINT(N);
  SLOWPRINT(E);
...
BEGIN
  ...
  PRINTTHEDIGS(K);
END;{:67}{69:}
PROCEDURE PRINTROMANIN(N:INTEGER);
...
END;{$ENDIF}
{:114}{119:}{292:}
PROCEDURE SHOWTOKENLIS(P,Q:INTEGER;L:INTEGER);
...
END;
{:694}{225:}
PROCEDURE PRINTSKIPPAR(N:INTEGER);
BEGIN
  CASE N OF 
    0: PRINTESC(376);
    1: PRINTESC(377);
...
BEGIN
  ...
  EQTB[P].HH.RH := E;
END;
PROCEDURE GEQWORDDEFIN(P:HALFWORD;W:INTEGER);
BEGIN
  EQTB[P].INT := W;
  XEQLEVEL[P] := 1;
END;
...
END;
{:519}{523:}
PROCEDURE PACKBUFFERED(N:SMALLNUMBER;A,B:INTEGER);
...
END;
{:829}{877:}
PROCEDURE POSTLINEBREA(FINALWIDOWPE:INTEGER);
...
{$IFDEF INITEX}
  ...
  WHILE J>0 DO
    BEGIN
      ...
      END;
{$IFDEF INITEX}
      TRIEUSED[K] := X+0;{$ENDIF}
      J := J-X;
      OPSTART[K] := J-0;
    END;
{$IFDEF INITEX}
  TRIENOTREADY := FALSE{$ENDIF}{:1325};
{1327:}
  BEGIN
    BEGIN
      GET(FMTFILE);
