          ...
          IF S=0 THEN{398:}
            BEGIN
              BEGIN
                ...
                print_str('Use of ');
              END;
              print_str(sprint_cs(WARNINGINDEX) + ' doesn''t match its definition');
              BEGIN
                HELPPTR := 4;
            ...
            BEGIN
              IF LONGSTATE=
                ...
                BEGIN
                  ...
                  END;
                  print_str(sprint_cs(WARNINGINDEX) + ' was complete');
                  BEGIN
                    HELPPTR := 3;
...
{$IFDEF STATS}
        ...
        ELSE{395:}
          BEGIN
            ...
            END;
            print_str(sprint_cs(WARNINGINDEX) + ' has an extra }');
            BEGIN
              HELPPTR := 6;
...
END;
{:405}{407:}
function scan_keyword_str(const s: shortstring): boolean;
VAR
  P: HALFWORD;
  Q: HALFWORD;
  i: Int32;
...
BEGIN
  ...
  while true do begin
    IF scan_keyword_str('width') THEN BEGIN
      SCANDIMEN(FALSE,FALSE,FALSE);
      MEM[Q+1].INT := CURVAL;
    end else if scan_keyword_str('height') THEN BEGIN
      SCANDIMEN(FALSE,FALSE,FALSE);
      MEM[Q+3].INT := CURVAL;
    end else if scan_keyword_str('depth') THEN BEGIN
      SCANDIMEN(FALSE,FALSE,FALSE);
      MEM[Q+2].INT := CURVAL;
    end else break;
  end;
              ...
              IF CURCMD<>6 THEN
                IF (CURTOK<=3120)OR(CURTOK>T)THEN
                  BEGIN
                    ...
                    END;
                    print_str(sprint_cs(WARNINGINDEX));
                    BEGIN
                      HELPPTR := 3;
...
{500:}
  WHILE TRUE DO BEGIN
    ...
    IF CONDPTR=SAVECONDPTR THEN BEGIN
      ...
      END;
      ERROR;
    END ELSE IF CURCHR=2 THEN BEGIN
      P := CONDPTR;
      IFLINE := MEM[P+1].INT;
      CURIF := MEM[P].HH.B1;
      IFLIMIT := MEM[P].HH.B0;
...
BEGIN
  ...
  IF scan_keyword_str('to') THEN begin
    SPECCODE := 0;
    SCANDIMEN(FALSE,FALSE,FALSE);
  end ELSE IF scan_keyword_str('spread') THEN begin
    SPECCODE := 1;
    SCANDIMEN(FALSE,FALSE,FALSE);
  end ELSE BEGIN
    SPECCODE := 1;
...
BEGIN
  ...
  ELSE IF scan_keyword_str('by') THEN;
  ARITHERROR := FALSE;
  IF Q<91 THEN{1238:}
    IF P<2 THEN
      BEGIN
...
BEGIN
  ...
  print_nl_str('! Font ' + sprint_cs(U) + '=' + RemoveFileExtension(FileName));
