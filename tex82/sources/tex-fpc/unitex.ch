...
[8] \marke J
check first program argument insted of compile time flags
to distinguish between INITEX and TEX.
...
if fpc_param_count > 0 then begin
  ...
  if FirstArg = '-ini' then begin
    TeXVariation := 1;
    write_ln('INITEX detected');
  end else if FirstArg = '-trip' then begin
    write_ln('TRIPTEX detected');
    TeXVariation := 2;
  end else begin
    write_ln('TEX detected:', FirstArg);
    TeXVariation := 0;
  end;
end else begin
  write_ln('TEX without args detected');
  TeXVariation := 0;
end;
if TeXVariation>0 then begin
...
begin last := first; if TeXVariation = 0 then argc := 1 else argc := 2;
...
[891] \marke J Do only if INITEX
@x
begin @!init if trie_not_ready then init_trie;@+tini@;@/
@y
begin if (TeXVariation>0) and trie_not_ready then init_trie;@/
@z

[1251] \marke J Do only if INITEX
@x
...
[1332] \marke J Special initialization, only for INITEX, not for TEX
@x
...
@ \marke J
Decide if 0=TEX 1=INITEX or 2=TRIPTEX via command line.
