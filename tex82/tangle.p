...
XCHR:ARRAY[ASCIICODE]OF CHAR;{:13}{20:}OUTPUT:TEXTFILE;
...
{:18}{21:}REWRITE(OUTPUT);{:21}{26:}IF PARAMCOUNT<>4 THEN BEGIN WRITELN(
'Usage: tangle input.web input.ch output.p output.pool');HALT;END;
ASSIGN(PASCALFILE,PARAMSTR(3));ASSIGN(POOL,PARAMSTR(4));
REWRITE(PASCALFILE);REWRITE(POOL);
...
IF TEXTPTR>MAXTEXTS-3 THEN BEGIN WRITELN(OUTPUT);
...
OUTPUT);WRITE(OUTPUT,'! Section ended in mid-comment');ERROR;END;
LOC:=LOC-1;GOTO 10;
...
WRITELN(OUTPUT,'This is TANGLE, Version 4.6');{183:}PHASEONE:=TRUE;
...
WRITE(OUTPUT,'Writing the output file');END;FLUSH(OUTPUT);
...
FOR II:=1 TO 2 DO WRITE(OUTPUT,'+',MAXTOKPTR[II]:1);
WRITE(OUTPUT,' tokens.');[:186];}CLOSE(PASCALFILE);CLOSE(POOL);
CLOSE(WEBFILE);CLOSE(CHANGEFILE);
